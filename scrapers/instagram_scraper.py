import json
import re
from typing import List, Dict, Optional
from scrapers.base_scraper import BaseScraper
from config import PAGE_TIMEOUT


class InstagramScraper(BaseScraper):
    """Scraper logic specifically for Instagram public profiles and individual posts."""

    async def scrape_user_posts(self, username: str, max_posts: int = 20) -> List[Dict]:
        await self.rate_limiter.wait("instagram.com")
        posts = []

        try:
            page = await self._create_optimized_page()

            graphql_data = []
            async def handle_response(response):
                if "graphql/query" in response.url or "?query_hash=" in response.url:
                    try:
                        data = await response.json()
                        graphql_data.append(data)
                    except Exception:
                        pass

            page.on("response", handle_response)

            profile_url = f"https://www.instagram.com/{username}/"
            response = await page.goto(profile_url, timeout=PAGE_TIMEOUT, wait_until="domcontentloaded")

            if response and response.status == 404:
                print(f"[IG] User {username} not found.")
                return posts

            await self._human_like_delay()

            login_wall = await page.locator("text='Log In'").count()
            if login_wall > 0 and len(graphql_data) == 0:
                print(f"[IG] Hit login wall for {username}. Might need proxy switch.")

            await self._random_scroll(page)

            try:
                shared_data_script = await page.evaluate("() => { "
                    "for (let s of document.getElementsByTagName('script')) {"
                    "  if (s.innerHTML.includes('window._sharedData')) return s.innerHTML;"
                    "}"
                    "return null;"
                "}")

                if shared_data_script:
                    match = re.search(r'window\._sharedData\s*=\s*({.+?});', shared_data_script)
                    if match:
                        data = json.loads(match.group(1))
                        edges = data.get('entry_data', {}).get('ProfilePage', [{}])[0].get(
                            'graphql', {}).get('user', {}).get(
                            'edge_owner_to_timeline_media', {}).get('edges', [])

                        for edge in edges[:max_posts]:
                            node = edge.get('node', {})
                            shortcode = node.get('shortcode')

                            if shortcode:
                                caption = ""
                                try:
                                    caption = node['edge_media_to_caption']['edges'][0]['node']['text']
                                except Exception:
                                    pass

                                posts.append({
                                    "post_url": f"https://www.instagram.com/p/{shortcode}/",
                                    "post_id": shortcode,
                                    "author_username": username,
                                    "caption": caption,
                                    "views": node.get('video_view_count', 0),
                                    "likes": node.get('edge_liked_by', {}).get('count', 0),
                                    "comments": node.get('edge_media_to_comment', {}).get('count', 0),
                                    "shares": 0,
                                    "posted_at": str(node.get('taken_at_timestamp', '')),
                                    "platform": "instagram"
                                })
            except Exception as e:
                print(f"[IG] _sharedData extraction error: {e}")

            if not posts:
                print(f"[IG] Could not extract posts for {username} via primary method")

            await self.rate_limiter.report_success("instagram.com")

        except Exception as e:
            await self.rate_limiter.report_error("instagram.com")
            print(f"[IG Error] Failed scraping user {username}: {str(e)}")
        finally:
            await self._teardown_browser()

        return posts[:max_posts]

    async def scrape_post_metrics(self, post_url: str) -> Optional[Dict]:
        await self.rate_limiter.wait("instagram.com")
        metrics = None

        try:
            page = await self._create_optimized_page()
            await page.goto(post_url, timeout=PAGE_TIMEOUT, wait_until="domcontentloaded")
            await self._human_like_delay()

            meta_desc = await page.get_attribute("meta[name='description']", "content")
            if meta_desc:
                likes_match = re.search(r'([\d,]+)\s+likes', meta_desc, re.IGNORECASE)
                comments_match = re.search(r'([\d,]+)\s+comments', meta_desc, re.IGNORECASE)

                likes = int(likes_match.group(1).replace(',', '')) if likes_match else 0
                comments = int(comments_match.group(1).replace(',', '')) if comments_match else 0

                metrics = {
                    "views": 0,
                    "likes": likes,
                    "comments": comments,
                    "shares": 0
                }

            await self.rate_limiter.report_success("instagram.com")
        except Exception as e:
            await self.rate_limiter.report_error("instagram.com")
            print(f"[IG Error] Failed scraping post {post_url}: {str(e)}")
        finally:
            await self._teardown_browser()

        return metrics

    async def scrape_single_video(self, video_url: str) -> Optional[Dict]:
        """Scrape a single Instagram reel/post and extract author + metrics."""
        await self.rate_limiter.wait("instagram.com")
        result = None

        try:
            page = await self._create_optimized_page()
            await page.goto(video_url, timeout=PAGE_TIMEOUT, wait_until="domcontentloaded")
            await self._human_like_delay()

            author_username = ""
            views = 0
            likes = 0
            comments = 0
            shares = 0
            caption = ""
            video_id = ""

            # Extract video ID from URL
            match = re.search(r'instagram\.com/(?:reel|p)/([\w-]+)', video_url)
            if match:
                video_id = match.group(1)

            # Method 1: Try meta tags for author
            # og:title often has: "Username on Instagram: caption"
            og_title = await page.get_attribute("meta[property='og:title']", "content")
            if og_title:
                # Pattern: "Username on Instagram: ..."
                title_match = re.match(r'^(.+?)\s+on\s+Instagram', og_title)
                if title_match:
                    author_username = title_match.group(1).strip().lstrip('@')

            # Method 2: Try looking for the username link on the page
            if not author_username:
                try:
                    # Header section usually has the author link
                    header_link = await page.locator("header a[href*='/']").first.get_attribute("href")
                    if header_link:
                        parts = [p for p in header_link.strip('/').split('/') if p]
                        if parts:
                            author_username = parts[-1]
                except Exception:
                    pass

            # Method 3: Extract from page source
            if not author_username:
                try:
                    page_content = await page.content()
                    owner_match = re.search(r'"owner"\s*:\s*\{[^}]*"username"\s*:\s*"([^"]+)"', page_content)
                    if owner_match:
                        author_username = owner_match.group(1)
                except Exception:
                    pass

            # Extract metrics from meta description
            meta_desc = await page.get_attribute("meta[name='description']", "content")
            if meta_desc:
                likes_match = re.search(r'([\d,]+)\s+likes', meta_desc, re.IGNORECASE)
                comments_match = re.search(r'([\d,]+)\s+comments', meta_desc, re.IGNORECASE)
                views_match = re.search(r'([\d,]+)\s+views', meta_desc, re.IGNORECASE)

                if likes_match:
                    likes = int(likes_match.group(1).replace(',', ''))
                if comments_match:
                    comments = int(comments_match.group(1).replace(',', ''))
                if views_match:
                    views = int(views_match.group(1).replace(',', ''))

            # Try caption from og:description
            og_desc = await page.get_attribute("meta[property='og:description']", "content")
            if og_desc:
                # Often format: "N Likes, N Comments - Username (@handle) on Instagram: 'caption'"
                cap_match = re.search(r'on Instagram:\s*["\']?(.*)', og_desc, re.DOTALL)
                if cap_match:
                    caption = cap_match.group(1).strip().rstrip("'\"")

            # Try to get view count from page elements for reels
            if views == 0:
                try:
                    view_el = await page.locator("span:has-text('plays')").first.inner_text()
                    if view_el:
                        num_match = re.search(r'([\d,.]+[KMB]?)', view_el)
                        if num_match:
                            views = self._parse_compact_number(num_match.group(1))
                except Exception:
                    pass

            result = {
                "video_url": video_url,
                "video_id": video_id,
                "author_username": author_username,
                "caption": caption,
                "views": views,
                "likes": likes,
                "comments": comments,
                "shares": shares,
                "posted_at": "",
                "is_video": True,
                "platform": "instagram"
            }

            await self.rate_limiter.report_success("instagram.com")

        except Exception as e:
            await self.rate_limiter.report_error("instagram.com")
            print(f"[IG Error] Failed scraping video {video_url}: {str(e)}")
        finally:
            await self._teardown_browser()

        return result

    def _parse_compact_number(self, text: str) -> int:
        """Parse numbers like 1.2K, 3.4M, 5B into integers."""
        text = text.strip().replace(',', '')
        multiplier = 1
        if text.endswith('K'):
            multiplier = 1_000
            text = text[:-1]
        elif text.endswith('M'):
            multiplier = 1_000_000
            text = text[:-1]
        elif text.endswith('B'):
            multiplier = 1_000_000_000
            text = text[:-1]
        try:
            return int(float(text) * multiplier)
        except ValueError:
            return 0
